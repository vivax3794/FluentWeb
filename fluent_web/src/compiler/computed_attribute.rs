...
pub fn compile(
    ...
    let normal = normal_nodes.into_iter().map(|node| {
        compile_stmt(&node, &normal_attribute_value(&node.code), data)
    });
    let prop = prop_nodes.into_iter().map(|node| {
        compile_stmt(&node, &prop_attribute_value(&node.code), data)
    });
